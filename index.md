...
makes `taxlist` suitable for its implementation in interactive documents
using `rmarkdonw` and `knitr` (e.g. reports, manuscripts and
check-lists).
...
![](reference/figures/taxlist_model.png)  
**Figure:** Relational model for `taxlist` objects (see [Alvarez &
...
a top-down direction. We will use the function
[`add_concept()`](https://docs.ropensci.org/taxlist/reference/add_concept.md)
to add a new taxon. Note that the arguments `TaxonName`, `AuthorName`,
and `Level` are used to provide the name of the taxon, the authority of
the name and the taxonomic rank, respectively.
...
genus, we use again the function
[`add_concept()`](https://docs.ropensci.org/taxlist/reference/add_concept.md),
but this time we will also provide the ID of the parent taxon with the
argument `Parent`.
...
You may have realized that the function
[`summary()`](https://docs.ropensci.org/taxlist/reference/summary.md) is
applied to provide on the one side a display of meta-information for the
whole `taxlist` object, and on the other side to show a detail of the
taxa included in the object. In the later case adding the keyword
`"all"` as second argument, the summary will show a detailed information
...
which provides a better display on the hierarchical strucutre of
`taxlist` objects.
...
the two first columns are mandatory. Also note that the column
**Parent** is pointing to the concept IDs of the respective parent
taxon. To get the object, we just use the
[`df2taxlist()`](https://docs.ropensci.org/taxlist/reference/df2taxlist.md)
...
`` `r print_name(Easplist, 206)` ``, which will insert *Cyperus papyrus*
...
use `` `r print_name(Easplist, 206, second_mention=TRUE)` ``, which will
insert *C. papyrus* L. in your text.
...
life forms in the `Easplist`, we can use the function
...
using the function
[`tax2traits()`](https://docs.ropensci.org/taxlist/reference/tax2traits.md).
By this way we will make taxonomic ranks suitable for frequency
calculations.
...
![](reference/figures/genera_bar-1.png)
